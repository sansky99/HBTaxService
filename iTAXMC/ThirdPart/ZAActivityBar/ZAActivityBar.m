...
+(void) showWithStatus:(NSString*) status image:(UIImage*) icon duration:(NSTimeInterval)duration forAction:(NSString *)action
{
    [[ZAActivityBar sharedView] showWithStatus:status image:icon duration:duration forAction:action];
}
...
- (UILabel *) actionIndicatorLabel {
    if (!actionIndicatorLabel) {
		...
		actionIndicatorLabel.font = [UIFont systemFontOfSize:[UIFont smallSystemFontSize]];
...
- (UILabel *)stringLabel {
    if (!stringLabel) {
		...
		stringLabel.font = [UIFont systemFontOfSize:[UIFont systemFontSize]];
...
- (UIView *)barView {
    if(!barView) {
        ...
        barView.layer.cornerRadius = CORNOR_RADIUS;
