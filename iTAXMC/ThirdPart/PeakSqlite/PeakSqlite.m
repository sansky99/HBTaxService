...
{
  NSString *condition = [NSString stringWithFormat: @" AND %@ = %ld", self.primaryField, (long)primaryId];
...
-(NSInteger) insertWithSql:(NSString *)sql parameters:(NSArray *)params
{
  ...
  if([self.database executeUpdate:sql withArgumentsInArray:params]){
    lastId = (long)[self.database lastInsertRowId];
  }
  [self.database close];
  return lastId;
}
...
//    if (date == nil)
//    {
//        date = [NSDate dateWithTimeIntervalSince1970:3600*24*30];
//    }
//    return date;
//}
@end
