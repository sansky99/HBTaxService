...
CGFloat kHeightOfTopScrollView = 44.0f;
static const CGFloat kWidthOfButtonMargin = 16.0f;//按钮间隔
static const CGFloat kFontSizeOfTabButton = 17.0f;//按钮宽度
...
- (void)initValues
{
    ...
    if ([self.moduleID intValue]== 30000) {
        kHeightOfTopScrollView = 0.0f;
    }
        //创建顶部可滑动的tab
...
- (void)layoutSubviews
{
    ...
    if (_isBuildUI) {
        ...
        if (!self.showViewTitle) {
            [_rootScrollView setFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height)];
        }
...
{
    int tag = (int)_rootScrollView.contentOffset.x/self.bounds.size.width +100;
