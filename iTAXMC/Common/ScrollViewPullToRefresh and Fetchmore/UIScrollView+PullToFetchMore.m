...
#import "UIScrollView+PullToFetchMore.h"
...
#pragma mark -
#pragma mark - SYBlockProgressView
@interface SYBlockProgressView : UIView
...
@implementation SYBlockProgressView
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGContextSetRGBFillColor(context, 204/ 255.0f, 204/ 255.0f, 204/ 255.0f,0.3 + 0.7 * self.progress);//颜色（RGB）,透明度
...
#pragma mark -
#pragma mark - SYPullToFetchtMoreView

//高度45.0f
@interface SYPullToFetchMoreView()
...
@property (nonatomic,strong) SYBlockProgressView     *block_progress_view;      //进度条
...
@implementation SYPullToFetchMoreView
...
- (void)loadLayer
{
    ...
    self.block_progress_view = [[SYBlockProgressView alloc] initWithFrame:CGRectMake(x_offset,
                                                                                     30.0f,
                                                                                     progress_width,
                                                                                     progress_heihgt)];
...
#pragma mark -
#pragma mark - UITable PullToFetchMore

static char SY_Pull_To_Fetch_More_View_Key;

@implementation UITableView (PullToFetchMore)

@dynamic pull_to_fetch_more_view;

- (void)setPull_to_fetch_more_view:(SYPullToFetchMoreView *)pull_to_fetch_more_view
{
    [self willChangeValueForKey:@"pull_to_fetch_more_view"];
    objc_setAssociatedObject(self, &SY_Pull_To_Fetch_More_View_Key, pull_to_fetch_more_view, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    [self didChangeValueForKey:@"pull_to_fetch_more_view"];
}

- (SYPullToFetchMoreView *)pull_to_fetch_more_view
{
    return objc_getAssociatedObject(self, &SY_Pull_To_Fetch_More_View_Key);
}
...
- (void)setPullToFetchMoreEndBlock:(PullFetchMoreEndBlock)fetch_more_end_block
{
    if (nil == self.pull_to_fetch_more_view)
    {
        ...
        self.pull_to_fetch_more_view                      = [[SYPullToFetchMoreView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, 45.0f)];
